...
class RemoteProductDataSourceImp implements RemoteProductDataSource {
  ...
  @override
  Future<List<ProductModel>> getAllProducts() => getProductRefactor(AppData.allProductUrl);
  ...
  Future<ProductModel> getProduct(String id) async {
    List<ProductModel> result = await  getProductRefactor(AppData.allProductUrl, id);
    return result[0];
  }
  ...
  ///
  /// Throws [ServerException] if request is failed
  Future<List<ProductModel>> getProductRefactor(String url, [String? id]) async {
      try {
        late http.Response result;
        if (id != null){
          result = await client.get(Uri.parse('${AppData.baseUrl}/$id'),
           ...
           result = await client.get(Uri.parse(url),
              headers: AppData.jsonHeader);
        }
        if (result.statusCode == 200) {
          if (id != null){
